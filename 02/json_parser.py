...
def parse_json(
    ...
    список полей, которые необходимо обработать,
    список имён, которые нужно найти и функцию-обработчика имени,
    который срабатывает, когда в каком-либо поле
    было найдено ключевое имя. Если аргумент required_fields
    не задан, функция-обработчик вызывается для всех полей,
    в которых есть keywords. Если аргумент keywords не задан,
    функция-обработчик вызывается для всех слов полей required_fields.
    """
