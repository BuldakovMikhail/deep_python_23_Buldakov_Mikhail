...
# Генератор должен принимать на вход имя файла или файловый объект
# и список слов для поиска. Генератор перебирает строки файла
# и возвращает только те из них (строку целиком),
# где встретилось хотя бы одно из слов для поиска.
# Поиск должен выполняться по полному совпадению слова без учета регистра.

# Например, для строки из файла "а Роза упала на лапу Азора"
# слово поиска "роза" должно найтись, а "роз" или "розан" - уже нет.
...
def gen_selected_lines(file_name, search_words):
    ...
    if isinstance(file_name, str):
        with open(file_name, encoding="utf-8", mode="r") as src:
