...
class TestGenerator(unittest.TestCase):
    def test_find_one_line(self):
        ...
        with mock.patch('builtins.open', new_callable=mock.mock_open, read_data=text) as f:
    ...
    def test_find_full_match(self):
        lines = ["а розан упала на лапу Азора",
                 "а Роза упала на лапу Азора", "а роз упала на лапу Азора"]
        text = '\n'.join(lines)
    ...
    def test_pass_number_instead_of_file(self):
        ...
        with self.assertRaises(Exception) as context:
            next(gen)
        self.assertTrue(
            'Neither file or file object' in str(context.exception))
